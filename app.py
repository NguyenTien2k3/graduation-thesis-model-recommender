...
# LỖI ĐÃ SỬA: Đọc tên biến môi trường (HF_TOKEN) thay vì đọc chuỗi Token (sai)
# Lấy token từ biến môi trường (nếu có)
HF_TOKEN = os.environ.get("HF_TOKEN")

# Các URL tải model và CSV
# LƯU Ý QUAN TRỌNG: Cả Model và CSV Data đều có vẻ yêu cầu xác thực (lỗi 401).
...
def download_file_stream(url, save_path, name, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # THÊM TOKEN VÀO HEADER NẾU CÓ (Áp dụng cho cả Model và CSV Data)
            if HF_TOKEN:
                headers['Authorization'] = f'Bearer {HF_TOKEN}'
                if name == "Model":
                    logging.info(f"[{name}] Sử dụng Hugging Face Token để xác thực.")
            
            with requests.get(url, stream=True, timeout=60, headers=headers) as r:
                # Kiểm tra lỗi 401/403 cụ thể trước
                if r.status_code == 401 or r.status_code == 403:
                     # Thông báo lỗi rõ ràng hơn khi dùng token
                     auth_msg = "với Token" if HF_TOKEN else "không có xác thực"
                     logging.error(f"[{name}] ❌ LỖI QUYỀN TRUY CẬP ({r.status_code}) {auth_msg}: Vui lòng kiểm tra lại quyền truy cập hoặc token.")
                     r.raise_for_status() # Vẫn raise để kích hoạt logic retry/failure
...
def load_items():
    ...
    # Tải CSV đã được cấu hình để sử dụng Token nếu có.
...
def load_model():
    ...
    # Tải Model đã được cấu hình để sử dụng Token nếu có.
...
def get_top_k_recommendations(user_id, item_ids, model, k=10, blocked_items=None):
    ...
    for iid in valid_items:
        try:
            ...
            # Giả định model có hàm predict với uid và iid.
