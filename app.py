        ...
        try:
            ...
            with requests.get(url, stream=True, timeout=60, headers=headers) as r:
                ...
                if r.status_code == 401 or r.status_code == 403:
                    ...
                    logging.error(
                        f"[{name}] ❌ LỖI QUYỀN TRUY CẬP ({r.status_code}) {auth_msg}: Vui lòng kiểm tra lại quyền truy cập hoặc token."
                    )
                    ...
                    for chunk in r.iter_content(chunk_size=8192):
                        if chunk:
                            ...
                            if total_size:
                                ...
                                if (
                                    current_step > last_logged_step
                                    and current_step < 100
                                ):
...
def load_model():
    ...
    # Tải Model có thể cần xác thực (nếu model Private)
